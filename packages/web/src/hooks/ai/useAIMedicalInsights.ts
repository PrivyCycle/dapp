...
import { prepareCycleDataForAI, createMedicalPrompt } from '../../lib/ai/dataPreparation';
import { useEncryptedLogEntries } from '../data/useEncryptedLogEntries';
import { useCycleData } from '../data/useCycleData';

export const useAIMedicalInsights = (): {
  ...
  const generateInsights = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const prompt = createMedicalPrompt(cycleData);
      
      // Create cache key based on data
      const cacheKey = `medical-${JSON.stringify(cycleData)}`;
      ...
      // Get AI insights
      const response = await AIService.getMedicalInsights(prompt);
      ...
      if (response) {
        ...
        setError('Failed to generate medical insights');
      }
    } catch (err) {
      console.error('Error generating medical insights:', err);
